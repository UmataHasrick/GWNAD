...
dataset_wsl_fft_all = np.load(dataDir+"/wsc_test_set_GWAK_LM.npy")[:-5000]
np.random.shuffle(dataset_wsl_fft_all)
wsc_training_set = torch.load(dataDir+"/wsc_training_set_GWAK.json")
...
train_wsc = [False, False, True, True]
...
for cnt, ic in enumerate(cut_combination):
    ...
    training_set_ae[4] = dataset_wsl_fft_all.copy()
    # wsc_test_set = dataset_wsl_fft_all.copy()
    
    # ic = [int(x) for x in cnt2ic_2[cnt].split(' ')]
    stric = str(cnt)+"_"+ "ratio_scan" + "_" + ("".join(['T' if train_wsc[i] else 'F' for i in range(4)])) + suffix
    cutAE = ic.copy()
    ...
    for iCS in np.arange(2, 4):
        ...
        if train_wsc[iCS]:
            ...
            # the AE training set
            for iStep in np.arange(iCS+1, 5):
    ...
    wsc_training_set[3] = training_set_ae[4].copy()

    wscs_last = trainSeriesSupC_struct(wsc_training_set, [202, 64, 16, 4], outputDir+"/WSC_training_figures/finalwsc_"+stric+".png")
